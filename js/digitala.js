document.addEventListener("DOMContentLoaded", function () {
  ...
  btnTwoProducts.addEventListener("click", function (event) {
    ...
    const isHidden =
      document.querySelector(".hide-2-product").style.display === "none";
  ...
  btnThreeProducts.addEventListener("click", function (event) {
    event.preventDefault();
    const isHidden =
      document.querySelector(".hide-3-product").style.display === "none";
...
function showWarningBiayaAdmin(input) {
  if (!warned) {
    if (
      confirm(
        "Anda ingin mengedit Total Biaya Admin? Perubahan akan mengabaikan perhitungan otomatis."
      )
    ) {
      warned = true;
...
function showWarning(input) {
  if (!warned) {
    if (
      confirm(
        "Anda ingin mengedit Total COD? Perubahan akan mengabaikan perhitungan otomatis."
      )
    ) {
      warned = true;
      input.readOnly = false; // Membiarkan pengguna mengedit
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch(
    ...
    .then((response) => response.json())
    .then((data) => {
      const sortedProvinsi = [
        ...new Set(data.alamat.map((item) => item[0])),
      ].sort();
  ...
  function handleDependentDropdown(alamat) {
    ...
    $("#provinsi").on("change", function () {
      ...
      kabupatenDropdown.html(
        '<option value="" disabled selected hidden>Pilih Kabupaten/Kota</option>'
      );
      kecamatanDropdown.html(
        '<option value="" disabled selected hidden>Pilih Kecamatan</option>'
      );

      const filteredKabupaten = alamat
        .filter((item) => item[0] === selectedProvinsi)
        .map((item) => item[1]);
      const sortedKabupaten = [...new Set(filteredKabupaten)].sort((a, b) =>
        a.localeCompare(b)
      );
    ...
    $("#kabupaten_kota").on("change", function () {
      ...
      const filteredKecamatan = alamat
        .filter((item) => item[1] === selectedKabupaten)
        .map((item) => item[2]);
      const sortedKecamatan = [...new Set(filteredKecamatan)].sort((a, b) =>
        a.localeCompare(b)
      );
  ...
  form.addEventListener("submit", function (event) {
    ...
    $("#kabupaten_kota")
      ...
      .trigger("change"); // Reset Kabupaten/Kota dropdown
    $("#kecamatan")
      .html(
        '<option value="" disabled selected hidden>Pilih Kecamatan</option>'
      )
      .trigger("change"); // Reset Kecamatan dropdown
